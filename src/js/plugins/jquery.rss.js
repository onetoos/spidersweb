  ...
  RSS.prototype.getTokenMap = function(entry) {
    ...
    return $.extend({
      ...
      teaserImageUrl: (function(entry) {
        try { return entry.content.match(/(<img.*?>)/gi)[0].match(/src="(.*?)"/)[1].replace(/image|original/i, _self.options.thumbnailSize) }
        catch(e) { return "" }
      })(entry),
      ...
      featuredImageUrl: (function(entry) {
        ...
        catch(e) { return entry.content.match(/(<img.*?>)/gi)[0].match(/src="(.*?)"/)[1].replace(/image|original/i, _self.options.thumbnailSize) }
      })(entry),
